...
class ResultWeight {
    ...
    public:
        ...
        vector<int> histogram;
        vector<float> weight;
        vector<int> setsWeight;

        ResultWeight(SetSystem setsys, vector<int> histo, vector<float> weights, vector<int> sw){
            ss = setsys;
            histogram = histo;
            weight = weights;
            setsWeight = sw;
...
SetSystem Grid(int n, int d){
    ...
    for (int dim = 0; dim < d; dim++){
        for (int i = 0; i < pow(n,1/d); i++){
            temp.clear();
            for (const Point& pt : p) {
                if (pt.coordinates[dim] > i/(pow(n,1/d))){
                    temp.push_back(1);
                } else {
                    temp.push_back(0);
                }
